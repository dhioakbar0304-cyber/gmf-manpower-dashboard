        ...
        for _, row in merged.iterrows():
            ...
            if stat_old.lower() != stat_new.lower():
                st.session_state.activity_logs.insert(0, {
                    ...
                    "msg": f"🔄 Status <b>{row.get('Nama_new', 'Personel')}</b> berubah: <span style='color:#F59E0B;'>{stat_old}</span> ➡️ <span style='color:#10B981;'>{stat_new}</span>"
                })
    ...
    for lok, koordinat in DOKUMEN_KOORDINAT.items():
        ...
        if not sub_df.empty:
            ...
            for _, row in sub_df.iterrows():
                ...
                s_html = f"<span style='color:#10B981;font-weight:700;'>🟢 Active</span>" if sv.lower() == 'active' else (f"<span style='color:#F59E0B;font-weight:700;'>🟡 Standby</span>" if sv.lower() == 'standby' else f"<span style='color:#EF4444;font-weight:800;animation:blinker 1s linear infinite;'>🔴 {sv}</span>")
                popup_html += f"<tr><td style='padding:4px; border-bottom:1px solid #e2e8f0;'>{row['Nama']}</td><td style='padding:4px; border-bottom:1px solid #e2e8f0;'>{s_html}</td></tr>"
